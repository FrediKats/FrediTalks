...
SCSSLint::RakeTask.new do |t|
  f = Tempfile.new(['freditalks-', '.scss'])
...
task spell: [:build] do
  Dir['_site/**/*.html'].each do |f|
    ...
    tmp = Tempfile.new(['freditalks-', '.txt'])
    tmp << text
    tmp.flush
    tmp.close
    stdout = `cat "#{tmp.path}" \
...
task orphans: [:build] do
  ...
  links = links
          .map { |a| a.gsub(%r{^/}, 'http://localhost.com') }
          .select { |a| a.start_with? 'http://localhost.com' }
          .map { |a| a.gsub(/#.*/, '') }
  links += Dir['_site/**/*.html']
           .map { |f| f.gsub('_site', 'http://localhost.com') }
  counts = {}
  links
